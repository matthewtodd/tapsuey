...
A <tt>`cap tapsuey:db:pull`</tt> task is provided so you can sync your staging environment with production when you deploy. (A guard clause is included so that the task has no effect in production.)
